...
```{python}
# Colors for V genes
colors = ["#fafa70","#fdef6b","#ffe566","#ffda63","#ffd061","#ffc660","#ffbb5f","#fdb15f","#fba860","#f79e61",
          "#f39562","#ef8c63","#e98365","#e37b66","#dd7367","#d66b68","#ce6469","#c65e6a","#bd576b","#b4526b",
          "#ab4c6b","#a1476a","#974369","#8c3e68","#823a66","#773764","#6d3361","#62305e","#572c5a","#4d2956"]

# Reshape data: Melt dataframe
...
# Add total V gene count per sample
...
assert all(v_family_long.groupby(['patient_id', 'timepoint', 'origin'])['proportion'].sum().round(5) == 1), "Proportion sums incorrect!"

# Loop over patients to create separate plots
for patient in v_family_long['patient_id'].unique():
    ...
    fig.update_layout(
        ...
        xaxis=dict(title_text="Timepoint - Origin"),
        yaxis=dict(title_text="Proportion"),
        barmode="stack",
    )

    # Add each V gene as a separate bar trace
    for g, c in zip(current['v_gene'].unique(), colors):
        plot_df = current[current['v_gene'] == g]
        fig.add_trace(
            go.Bar(
                x=plot_df['timepoint'] + " - " + plot_df['origin'],  # Fix x-axis issue
                y=plot_df['proportion'],
                name=g,
                marker_color=c,
                text=str(plot_df['total_v_genes'].iloc[0]) if g == 'TCRBV30' else "",  # Fix text issue
                textposition='outside' if g == 'TCRBV30' else None
            )
        )
